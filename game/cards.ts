...
export function actions(move = 1, options: { fly?: number; attack?: readonly [number, number?]; defense?: readonly [number, number]; magic?: readonly [number, number]; cannon?: readonly [number, number]; /** M🫸N: push M hexes, select a target up to N hexes away. */ push?: readonly [number, number]; /** M❤️N: mend M health at distance N. */ mending?: readonly [number, number]; /** M🔮N: temporarily raise an active's left/right numbers. */ upgrade?: { left?: number; right?: number; range: number } } = {}): readonly TroopAction[] {
