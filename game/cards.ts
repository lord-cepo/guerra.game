...
/** M💣N — place an inert M-damage bomb on a hex at distance N. */
...
export type ContinuousEffect =
  ...
  | { condition: 'deployed'; kind: 'ability-bonus'; ability: 'attack' | 'magic'; left?: number; right?: number; label: string };

/** A one-shot event and the state change resolved each time it occurs. */
export type EventCondition = 'start' | 'end' | 'opponentStart' | 'opponentEnd' | 'bashAttack' | 'bashDefense' | 'bashRetreat' | 'bash' | 'magicUsed' | 'successfulAttack' | 'movementUsed' | 'death';
export type EventResolution =
  | { kind: 'heal'; amount: number }
  /** A permanent, stackable magenta upgrade produced by a resolved event. */
  | { kind: 'magenta-upgrade'; ability: 'attack'; left?: number; right?: number }
  | { kind: 'permanent-damage'; amount: number }
  | { kind: 'death-attack'; damage: number; range: number }
  | { kind: 'revive' }
  | { kind: 'optional-move'; distance: number };
export interface EventEffect { condition: EventCondition; resolution: EventResolution; }

export type TroopAction = MoveAction | FlyAction | AttackAction | MagicAction | DefenseAction | CannonAction | BombAction | PushAction | MendingAction | UpgradeAction;
...
export interface TroopSeed {
  ...
  eventEffects?: readonly EventEffect[];
}
...
function actions(move = 1, options: ActionOptions = {}): readonly TroopAction[] {
  ...
  if (options.bomb) result.push({ type: 'bomb', damage: options.bomb[0], range: options.bomb[1] });
...
export const troopSeeds: readonly TroopSeed[] = [
  ...
  { id: 'squirrel-king', name: 'Squirrel King', role: 'hero', baseHealth: 4, deploymentRegions: ['starting'], actions: actions(1, { magic: [1, 3] }), eventEffects: [{ condition: 'magicUsed', resolution: { kind: 'heal', amount: 1 } }], passiveDescription: '🔥: +1❤️', ruleDescription: 'Kindle: heals 1 permanent damage each time it uses Magic.' },
  { id: 'tiger-queen', name: 'Tiger Queen', role: 'hero', baseHealth: 6, deploymentRegions: ['starting'], actions: actions(2) },
  { id: 'wandering-monarch', name: 'Wandering Monarch', role: 'hero', baseHealth: 3, deploymentRegions: ['starting'], actions: actions(1, { push: [1, 2], defense: [2, 1] }), eventEffects: [{ condition: 'end', resolution: { kind: 'optional-move', distance: 1 } }], passiveDescription: 'End: 🥾1', ruleDescription: 'End stride: at the end of your turn, you may move this hero 1 hex or decline and finish the turn.' },
  ...
  { id: 'reed-archer', name: 'Reed Archer', role: 'troop', baseHealth: 1, deploymentRegions: ['intermediate'], actions: actions(0, { attack: [4, 1] }), eventEffects: [{ condition: 'successfulAttack', resolution: { kind: 'magenta-upgrade', ability: 'attack', left: 1, right: 0 } }], passiveDescription: '🏹✓: +1🏹', ruleDescription: 'Sharpen: after its ranged attack successfully inflicts damage, it permanently gains a stackable magenta +1 ranged damage.' },
  ...
  { id: 'marching-giant', name: 'Marching Giant', role: 'troop', baseHealth: 5, deploymentRegions: ['starting'], actions: actions(1), eventEffects: [{ condition: 'movementUsed', resolution: { kind: 'permanent-damage', amount: 1 } }], passiveDescription: '🥾: -1❤️', ruleDescription: 'Attrition: suffers 1 permanent damage each time it completes a Move action.' },
  { id: 'phoenix-moth', name: 'Phoenix Moth', role: 'troop', baseHealth: 2, deploymentRegions: ['intermediate'], actions: actions(0, { fly: 2 }), eventEffects: [{ condition: 'death', resolution: { kind: 'death-attack', damage: 3, range: 2 } }], passiveDescription: '💀: 3🏹2', ruleDescription: 'Death burst: when this troop dies, choose an enemy within 2 hexes of its death hex to receive 3 physical damage.' },
  { id: 'cherub-sprout', name: 'Cherub Sprout', role: 'troop', baseHealth: 1, deploymentRegions: ['starting'], actions: actions(2), eventEffects: [{ condition: 'death', resolution: { kind: 'revive' } }], passiveDescription: '💀: 👼', ruleDescription: 'Revive: when this troop dies, choose one of your defeated troops and make it available to deploy again. If none are defeated, nothing happens.' },
