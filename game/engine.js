export function createGameState() { return { activePlayer: 1, units: [], effects: [], bashes: [], lastActingTroopId: {}, defeatedTroopIds: [], revision: 0, events: [], triggerEvents: [] }; }
export function unitId(unit) { return unit.id ?? `${unit.owner}:${unit.troopId}`; }
function findUnit(state, id) { return state.units.find(unit => unit.id === id || unitId(unit) === id || (!unit.id && unit.troopId === id)); }
function matchesUnitId(unit, id) { return unit.id === id || unitId(unit) === id || (!unit.id && unit.troopId === id); }
export function health(unit, cards) { return Math.max(0, (cards.get(unit.troopId)?.baseHealth ?? 0) - unit.permanentDamage); }
export function combatBreakdown(state, troopId, cards, contestedCoordinate) {
    const unit = findUnit(state, troopId);
    if (!unit)
        throw new Error('Troop is not on the board.');
...
function card(cards, id) { const result = cards.get(id); if (!result)
    throw new Error('Unknown troop.'); return result; }
function at(state, coordinate) { return state.units.find(unit => unit.coordinate === coordinate); }
function upgradeBonus(unit, ability) { return (unit.upgrades ?? []).filter(upgrade => upgrade.ability === ability || upgrade.ability === undefined).reduce((total, upgrade) => ({ left: total.left + (upgrade.left ?? 0), right: total.right + (upgrade.right ?? 0) }), { left: 0, right: 0 }); }
function moveRange(troop, unit) { return (troop.actions.find(action => action.type === 'move')?.maxDistance ?? 0) + upgradeBonus(unit, 'move').right; }
function flyRange(troop, unit) { return (troop.actions.find(action => action.type === 'fly')?.maxDistance ?? 0) + upgradeBonus(unit, 'fly').right; }
function range(troop, unit, type) { const action = troop.actions.find(item => item.type === type); return action && 'range' in action ? action.range + upgradeBonus(unit, type).right + (type === 'attack' ? unit.rangedRangeBonus ?? 0 : 0) : -1; }
function attackDamage(troop, unit, cards) {
    const action = troop.actions.find((item) => item.type === 'attack');
    return (action?.damage ?? health(unit, cards)) + (unit.rangedDamageBonus ?? 0) + upgradeBonus(unit, 'attack').left;
}
function value(troop, type, cards, unit) {
    if (type === 'magic')
        return (troop.actions.find((item) => item.type === 'magic')?.damage ?? health(unit, cards)) + upgradeBonus(unit, 'magic').left;
    return (troop.actions.find((item) => item.type === 'defense')?.block ?? health(unit, cards)) + upgradeBonus(unit, 'defense').left;
}
function cannonDamage(troop, unit) { return (troop.actions.find((item) => item.type === 'cannon')?.damage ?? 0) + upgradeBonus(unit, 'cannon').left; }
...
function spendUpgrade(unit, action) { if (['move', 'fly', 'defense', 'self-defense', 'push', 'mending', 'upgrade'].includes(action.type))
    unit.upgrades = []; }
function distance(a, b) { const [ax, ay] = a.split(',').map(Number); const [bx, by] = b.split(',').map(Number); return Math.max(Math.abs(ax - bx), Math.abs(ay - by), Math.abs((ax - ay) - (bx - by))); }
function cannonLine(from, to, maxDistance) {
    ...
    const line = Array.from({ length: steps }, (_, index) => `${fromX + stepX * (index + 1)},${fromY + stepY * (index + 1)}`);
...
function pushLine(from, target, maxDistance) {
    const [fromX, fromY] = from.split(',').map(Number);
    const [targetX, targetY] = target.split(',').map(Number);
    const dx = targetX - fromX;
    const dy = targetY - fromY;
    const steps = distance(from, target);
    if (!steps || !(dx === 0 || dy === 0 || dx === dy))
        return undefined;
    const stepX = dx === 0 ? 0 : dx / Math.abs(dx);
    const stepY = dy === 0 ? 0 : dy / Math.abs(dy);
    const line = Array.from({ length: maxDistance }, (_, index) => `${targetX + stepX * (index + 1)},${targetY + stepY * (index + 1)}`);
    ...
    return valid(line.at(-1)) ? line : undefined;
}
function neighbours(coordinate) { const [x, y] = coordinate.split(',').map(Number); return [[1, 0], [-1, 0], [0, 1], [0, -1], [1, 1], [-1, -1]].map(([dx, dy]) => `${x + dx},${y + dy}`); }
function hasFreePath(state, from, to, maxDistance) {
    ...
    while (queue.length) {
        ...
        for (const next of neighbours(current)) {
            if (!valid(next) || visited.has(next))
                continue;
            if (next === to)
                return true;
            if (at(state, next))
...
function valid(c) { const match = c.match(/^(-?\d+),(-?\d+)$/); if (!match || c === '0,0')
    return false; const [x, y] = match.slice(1).map(Number); return x >= -3 && x <= 3 && y >= -4 && y <= 4 && x - y >= -3 && x - y <= 3; }
const p1Start = new Set(['1,2', '1,3', '1,4', '2,3', '2,4', '3,4']);
const p1Middle = new Set(['0,1', '0,2', '0,3', '-1,1', '-2,1', '-1,2']);
const p1Side = new Set(['1,1', '2,1', '3,1', '2,2', '3,2', '3,3']);
const flip = (coordinate) => { const [x, y] = coordinate.split(',').map(Number); return `${-x},${-y}`; };
function region(c) { if (p1Start.has(c))
    return { id: 'p1-start', type: 'starting', home: 1 }; if ([...p1Start].map(flip).includes(c))
    return { id: 'p2-start', type: 'starting', home: 2 }; if (p1Middle.has(c))
    return { id: 'p1-middle', type: 'intermediate', home: 1 }; if ([...p1Middle].map(flip).includes(c))
    return { id: 'p2-middle', type: 'intermediate', home: 2 }; if (p1Side.has(c))
    return { id: 'p1-side', type: 'intermediate', home: 1 }; if ([...p1Side].map(flip).includes(c))
    return { id: 'p2-side', type: 'intermediate', home: 2 }; return { id: 'front', type: 'front' }; }
export function controlAt(state, coordinate, cards, virtual) { const target = region(coordinate); let one = target.home === 1 ? .5 : 0; let two = target.home === 2 ? .5 : 0; const units = virtual ? [...state.units.filter(unit => unitId(unit) !== unitId(virtual)), virtual] : state.units; for (const unit of units) {
    if (region(unit.coordinate).id !== target.id)
        continue;
    if (unit.owner === 1)
        one += health(unit, cards);
    else
        two += health(unit, cards);
} return { ...target, playerOne: one, playerTwo: two, controller: one === two ? undefined : one > two ? 1 : 2 }; }
export function controlSummary(state, cards) {
    return Object.fromEntries(['1,2', '-1,-2', '0,1', '0,-1', '1,1', '-1,-1', '1,0'].map(coordinate => {
        ...
        const bash = state.bashes.find(item => region(item.target).id === region(coordinate).id);
        const attacker = bash ? findUnit(state, bash.attackerId) : undefined;
        const summary = controlAt(state, coordinate, cards, attacker ? { ...attacker, coordinate: bash.target } : undefined);
        return [summary.id, summary];
    }));
}
function controller(state, coordinate, cards, virtual) { return controlAt(state, coordinate, cards, virtual).controller; }
function shieldBonus(unit, effect) { return (unit.troopId === 'p2-2' || unit.troopId === 'river-otter') && effect.owner === unit.owner && (effect.sourceUnitId ?? `${effect.owner}:${effect.sourceTroopId}`) !== unitId(unit) ? 1 : 0; }
function modifier(state, unit, cards, virtual) {
    ...
    if (bash) {
        ...
        if (findUnit(state, opponentId)?.troopId === 'canyon-hawk')
            return 0;
    }
...
function remove(state, unit, cards) { state.units = state.units.filter(item => item !== unit); state.defeatedTroopIds ??= []; state.defeatedTroopIds.push(unitId(unit)); if (card(cards, unit.troopId).role === 'hero')
    state.winner = unit.owner === 1 ? 2 : 1; }
const passiveHandlers = {
    ...
    magicUsed: { 'p2-hero': (state, unit, event) => {
            if (event.actingTroopId === unit.troopId)
                unit.permanentDamage = Math.max(0, unit.permanentDamage - 1);
        }, 'squirrel-king': (state, unit, event) => {
            if (event.actingTroopId === unit.troopId)
                unit.permanentDamage = Math.max(0, unit.permanentDamage - 1);
        } },
    bashAttack: { 'sahel-porcupine': (_state, unit, event) => {
            if (event.attackerId !== unitId(unit))
                return;
 ...
 */
export function dispatchTrigger(state, event, cards, listenerOwners = [1, 2]) {
    state.triggerEvents ??= [];
    ...
    const units = state.units.filter(unit => listenerOwners.includes(unit.owner)).sort((left, right) => {
        const leftHero = card(cards, left.troopId).role === 'hero';
        const rightHero = card(cards, right.troopId).role === 'hero';
...
export function registerTimedPassive(timing, troopId, handler) {
    return registerPassive(timing, troopId, (state, unit, event) => handler(state, unit, event.actingTroopId));
}
export function runTimedPassives(state, player, timing, cards, actingTroopId) {
...
export function applyGameAction(before, player, action, cards) {
    const state = structuredClone(before);
    if (state.winner || state.activePlayer !== player)
        throw new Error('It is not your turn.');
    if (action.type === 'pass') {
        const turnEvent = { player, troopIds: [] };
    ...
    if (state.lastActingTroopId?.[player] === action.troopId)
        throw new Error('This troop acted on your previous turn.');
    if (action.type !== 'self-defense' && !valid(action.coordinate))
        throw new Error('Invalid hex.');
    const troop = card(cards, action.troopId);
    const unit = state.units.find(item => item.owner === player && item.troopId === action.troopId);
    let vacatedCoordinate;
    if (action.type === 'deploy') {
        if (unit || state.defeatedTroopIds?.includes(`${player}:${action.troopId}`) || at(state, action.coordinate))
            throw new Error('Deployment hex is unavailable.');
        ...
        if (!allowed)
            throw new Error('You do not control a valid deployment region.');
        if (troop.role !== 'hero' && !state.units.some(item => item.owner === player && card(cards, item.troopId).role === 'hero'))
            throw new Error('Deploy your hero first.');
        ...
        if (!unit)
            throw new Error('Your troop is not deployed.');
        if (action.type === 'move' || action.type === 'fly') {
            const maxDistance = action.type === 'move' ? moveRange(troop, unit) : flyRange(troop, unit);
            if (distance(unit.coordinate, action.coordinate) > maxDistance || (action.type === 'move' && !hasFreePath(state, unit.coordinate, action.coordinate, maxDistance)))
                throw new Error(action.type === 'fly' ? 'Flight destination is out of range.' : 'Move has no free path.');
            ...
            if (!target) {
                ...
                vacatedCoordinate = unit.coordinate;
                if (state.bashes.some(bash => bash.target === action.coordinate))
                    throw new Error('A bash is already happening on this hex.');
                ...
                const bashEvent = { trigger: 'bash', player, hex: action.coordinate, troopIds: [attackerId, defenderId], actingTroopId: action.troopId, attackerId, defenderId };
                ...
                throw new Error('A friendly troop occupies this hex.');
        }
            ...
            if (!pushed || !line || action.destination !== line.at(-1))
            ...
            else if (landingOccupant.owner === pushed.owner)
                throw new Error('A pushed troop cannot land on a friendly troop.');
            else {
                if (state.bashes.some(bash => bash.target === action.destination))
                    throw new Error('A bash is already happening on this hex.');
                const attackerId = unitId(pushed);
                const defenderId = unitId(landingOccupant);
                state.bashes.push({ attackerId, defenderId, target: action.destination });
                const bashEvent = { trigger: 'bash', player, hex: action.destination, troopIds: [attackerId, defenderId], actingTroopId: action.troopId, attackerId, defenderId };
            ...
            if (actionRange < 0 || distance(unit.coordinate, action.coordinate) > actionRange)
            ...
            if (isOffensive && targetUnit?.owner === player && !isBashTarget)
                throw new Error('Cannot target a friendly troop.');
            if (action.type === 'cannon') {
                const line = cannonLine(unit.coordinate, action.coordinate, actionRange);
                if (!line)
                    throw new Error('Cannon target must be in a straight line.');
                for (const target of line)
                    state.effects.push({ owner: player, sourceTroopId: action.troopId, sourceUnitId: unitId(unit), kind: 'cannon', target, value: cannonDamage(troop, unit) });
            }
            else {
                if (action.type === 'attack')
                    state.effects.push({ owner: player, sourceTroopId: action.troopId, sourceUnitId: unitId(unit), kind: 'attack', target: action.coordinate, value: attackDamage(troop, unit, cards) });
                else if (action.type === 'mending') {
                    const target = at(state, action.coordinate);
                    if (!target || target.owner !== player)
                        throw new Error('Mending must target a friendly troop.');
                    const mending = troop.actions.find((item) => item.type === 'mending');
                    target.permanentDamage = Math.max(0, target.permanentDamage - (mending?.amount ?? 0) - upgradeBonus(unit, 'mending').left);
                }
                else if (action.type === 'upgrade') {
                    const target = at(state, action.coordinate);
                    if (!target || target.owner !== player)
                        throw new Error('Upgrade must target a friendly troop.');
                    if (card(cards, target.troopId).role === 'temple')
                        throw new Error('A temple cannot be upgraded.');
                    const upgrade = troop.actions.find((item) => item.type === 'upgrade');
                    target.upgrades ??= [];
                    ...
                    if (!targetAction || !useful)
                        throw new Error('This upgrade cannot affect the selected ability.');
                    target.upgrades.push({ ability: action.ability, left: upgrade?.left, right: upgrade?.right, sourceUnitId: unitId(unit) });
                }
                else
                    state.effects.push({ owner: player, sourceTroopId: action.troopId, sourceUnitId: unitId(unit), kind: action.type, target: action.coordinate, value: value(troop, action.type, cards, unit) });
            }
        }
    }
    if (unit)
        spendUpgrade(unit, action);
...
function resolveAfterDefenderAction(state, defender, cards) {
    const resolvedUpgradeSources = new Set();
    for (const effect of state.effects.filter(item => item.owner !== defender && (item.kind === 'attack' || item.kind === 'cannon'))) {
        if (effect.sourceUnitId)
            resolvedUpgradeSources.add(effect.sourceUnitId);
        const unit = at(state, effect.target);
        if (unit && unit.owner === defender) {
            unit.permanentDamage += Math.max(0, effect.value - modifier(state, unit, cards));
            if (health(unit, cards) === 0)
                remove(state, unit, cards);
        }
    }
    for (const effect of state.effects.filter(item => item.owner !== defender && item.kind === 'magic')) {
        if (effect.sourceUnitId)
            resolvedUpgradeSources.add(effect.sourceUnitId);
        const unit = at(state, effect.target);
        if (unit && unit.owner === defender && health(unit, cards) <= effect.value)
            remove(state, unit, cards);
    }
    for (const bash of [...state.bashes]) {
        ...
        if (!attacker || !originalDefender) {
            state.bashes = state.bashes.filter(item => item !== bash);
            continue;
        }
        if (originalDefender.owner !== defender)
            continue;
        ...
        if (attackerPower === defenderPower) {
            remove(state, attacker, cards);
            remove(state, target, cards);
        }
        else {
            ...
            if (winner === attacker && state.units.includes(winner))
                winner.coordinate = bash.target;
        }
        state.bashes = state.bashes.filter(item => item !== bash);
    }
