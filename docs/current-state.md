...
- Board geometry, descriptions, cards, grid construction, staged projections, deck-builder state, application screens, animation timing/projectiles, and authoritative revision differences live in focused client modules; `hex-grid.ts` currently retains match rendering and interaction orchestration.
...
- Gore uses the emoji-rendered `M🐏N`: confirmation leaves the troop at its origin while `horns.png` repeats along the selected aligned line through the opponent's turn. After the opponent acts, the troop moves to the endpoint and deals modifier-aware physical damage to every enemy recorded along its path; a surviving enemy at the endpoint starts a bash. Each damaging hit independently activates hit passives, so Ironhide Boar Pup gains +1 physical and +1 magic modifier per troop damaged. The line may cross friendly troops, but cannot end on one, and movement plays before Gore slash/bash resolution.
