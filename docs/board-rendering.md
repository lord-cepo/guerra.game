...
All troops use the full `N ♥ M` form, where `N` is current health and `M` is total/base health, even when both values match. The signed combat modifier remains directly below health and moves with the troop's rigid visual group.
...
Rewind inspection is suspended while a local troop selection is active or awaiting its server echo. Selecting a troop redirects any open rewind toward the authoritative endpoint and prevents further rewind entry until the selection is cleared, so action targeting (especially Bash) cannot visually move an inactive enemy away from its real hex.
...
- Confirmation preserves the staged result for the acting player without replay. Both players' currently inactive troops resolve their own inspection independently. If the local troop's most recent action was Deploy, its first pointer exit after the action only arms inspection, preventing the still-hovered confirmation hex from rewinding immediately. A remote opponent action is armed immediately because this browser did not confirm it with the pointer; a local action also becomes automatically armed as soon as a newer authoritative action exists. Hover can switch directly between the two inactive troops without a completed inspector blocking the other. Later pointer entry runs the deployment slider toward `0`; pointer exit runs toward `1`. Changing direction samples the current position without snapping, then starts a fresh fast-start/slow-finish leg toward the new endpoint rather than literally reversing the original easing curve.
...
- Every currently inactive troop—local and opposing—resolves inspection from its own most recent action rather than the match's globally latest event. If that action was Move/Fly, its origin/destination dissolve is controlled by one slider: `0` shows the origin and `1` shows the authoritative destination. The local action's first exit arms inspection without moving the troop; the opponent action is armed immediately. Subsequent hover runs toward `0`, pointer exit runs toward `1`, and rapid hover/exit/rehover changes direction from the current position with a fresh fast-start/slow-finish leg.
...
- Confirmation follows the same audience split as Move/Fly. After the first pointer exit arms inspection, hovering the inactive pushing troop controls the pushed troop on one straight-line slider: `0` is its pre-push hex and `1` is its authoritative destination. Pointer entry runs toward `0`, pointer exit runs toward `1`, and changing direction starts immediately from the current position with the same fast-start/slow-finish easing in either direction.
...
- Pull uses the same target selection, straight-line ease-out displacement, opponent-confirmed playback, and reversible hover replay as Push, but its landing line runs from the selected object toward the acting troop.
- Pull can target troops or bombs. A pull stops at the nearest legal hex before the acting troop and cannot land on a friendly troop; a troop landing on an enemy starts the normal bash flow.
...
- Hovering a currently stunned troop replays the same enlarged, gradual-fade stun animation over that troop's hex.
...
- A Boar Warlord's split-bash hex info shows its current `+N if ⚔️` bonus instead of the static `+1`; the row uses the existing magenta upgraded-effect treatment once its post-bash bonus has grown.
...
- Cannon resolution joins the delayed black-magic damage presentation for every occupied line hex, including friendly troops, visible nonlethal health loss, and the retained-card death sequence. It does not use the physical combat modifier and ignores shields under the current rules.

### Gore

- Gore uses `M🐏N`: the authoritative action moves the troop along a clear board-aligned line to the selected final hex, where a friendly occupant is invalid, an empty hex is allowed, and an enemy occupant starts the ordinary bash flow.
- The staged preview uses one upright `horns.png` head with Cannon's straight, repeating materialize/travel/impact cycle and no trail. On remote confirmation, the troop itself moves from the recorded origin to the target hex; its delayed enemy-only physical damage then uses the normal slash/health presentation after the opponent responds.
...
- A troop with a magic modifier shows a purple-filtered `🛡️` beside its modifier line. When the magic modifier increases, the renderer reuses the seven-frame shield sequence with the same purple filter; turn replay replays that sequence directly on the target hex without a flight.
...
- Permanent event modifiers are rendered in the modifier line: physical values use the existing magenta treatment, magic values use purple, and a combined value is shown as `+M+N`. Obsidian/Titanium are catalogue rules rather than separate board overlays.
- Triggered Frosthorn Pull choices use the existing Pull target/destination projection, while target-specific hit Stun is represented by the existing Stun effect and hover replay.
...
- Mending uses the Unicode mending-heart sequence `U+2764 U+FE0F U+200D U+1FA79` (`❤️‍🩹`) as SVG text.
...
- The target sweep reuses the 1.5-second slash reveal/hold/top-to-bottom disappearance timing, with a 62px mending heart replacing `slash.png` over the health region.
- After confirmation, the acting client does not replay the presentation. The opponent sees one heart flight with a one-shot trail, followed by one complete target sweep.
